...
const routesUtils = {
    ...
    setPartRanges(dataLocations, outerRange) {
        ...
        if (dataLocations.length === 1) {
            ...
            // If pre-md-model-version 2 data location (without size),
            // would not be in this function
            // since return not implemented in api. So ok that not
            // setting size property (size not used for get request)
        ...
        for (let i = 0; i < dataLocations.length; i++) {
            ...
            if (partStart >= begin) {
                ...
                // modify size to be stored for object put part copy
                partWithRange.size = Math.min(partSize,
                    endPart - startOffset + 1).toString();
